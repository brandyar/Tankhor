  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      if (product) {
        ...
        const cId = typeof product.category_id === 'number' ? product.category_id : (product.category_id as any)?.id || '';
        const colId = typeof product.collection_id === 'number' ? product.collection_id : (product.collection_id as any)?.id || '';
        const sId = typeof product.season_id === 'number' ? product.season_id : (product.season_id as any)?.id || '';
        const sgId = typeof product.size_guide_template_id === 'number' ? product.size_guide_template_id : (product.size_guide_template_id as any)?.id || '';
        setBrandId(bId);
        setCategoryId(cId);
