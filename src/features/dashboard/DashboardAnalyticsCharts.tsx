...
const CustomSalesTooltip = ({ active, payload, label, currency, isPersian }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-neutral-900 text-white p-3 rounded-lg shadow-xl border border-neutral-800 text-xs font-sans space-y-1 min-w-[160px]">
        <p className="text-neutral-400 font-mono text-[11px] pb-1 border-b border-neutral-800">
...
const CustomFlowTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-neutral-900 text-white p-3 rounded-lg shadow-xl border border-neutral-800 text-xs font-sans space-y-1.5 min-w-[150px]">
        <p className="text-neutral-400 font-mono text-[11px] pb-1 border-b border-neutral-800">
          تاریخ: {data.displayDate || label}
        </p>
...
const CustomCategoryTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-neutral-900 text-white p-2.5 rounded-lg shadow-xl border border-neutral-800 text-xs font-sans space-y-1">
...
export const DashboardAnalyticsCharts: React.FC<DashboardAnalyticsChartsProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-white p-3 sm:p-4 rounded-xl border border-neutral-200/80 shadow-xs">
        <div className="flex items-center gap-2 text-xs font-bold text-neutral-800">
          <Calendar className="w-4 h-4 text-neutral-500" />
          <span>بازه زمانی گزارشات و تحلیل‌ها:</span>
        </div>
        <div className="flex items-center gap-1 bg-neutral-100 p-1 rounded-lg w-full sm:w-auto">
          <button
              ...
              <div className="flex items-center gap-1 bg-neutral-100 p-0.5 rounded-lg text-xs">
                <button
                  ...
                  className={`px-2.5 py-1 rounded font-medium transition-all ${
                    salesMetricView === 'revenue' ? 'bg-white text-neutral-900 shadow-2xs font-bold' : 'text-neutral-600'
                  }`}
                >
                  ریالی
                </button>
                <button
                  ...
                  className={`px-2.5 py-1 rounded font-medium transition-all ${
                    salesMetricView === 'orders' ? 'bg-white text-neutral-900 shadow-2xs font-bold' : 'text-neutral-600'
                  }`}
                >
                  تعداد سفارش
                </button>
          ...
          <div className="h-72 w-full pt-4">
            {salesData.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-neutral-400 text-xs">
                <TrendingUp className="w-8 h-8 stroke-1 mb-2 text-neutral-300" />
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={salesData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                  ...
                  <Area
                    ...
                    stroke={salesMetricView === 'revenue' && canViewFinancials ? '#0f172a' : '#2563eb'}
                    strokeWidth={2.5}
                    fill={salesMetricView === 'revenue' && canViewFinancials ? 'url(#salesGrad)' : 'url(#ordersGrad)'}
                    dot={{ r: 3, fill: salesMetricView === 'revenue' && canViewFinancials ? '#0f172a' : '#2563eb' }}
        ...
        >
          <div className="h-72 w-full pt-4">
            {inventoryFlowData.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-neutral-400 text-xs">
                <Package className="w-8 h-8 stroke-1 mb-2 text-neutral-300" />
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={inventoryFlowData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                  ...
                  <Legend
                    ...
                    formatter={(val) => (val === 'inflow' ? 'ورود به انبار' : 'خروج از انبار')}
                  />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          <div className="h-64 w-full relative flex items-center justify-center">
            {totalCategoryStock === 0 ? (
              <div className="text-center text-neutral-400 text-xs">
                <PieIcon className="w-8 h-8 mx-auto mb-2 text-neutral-300 stroke-1" />
          ...
          <div className="mt-2 space-y-1.5 max-h-36 overflow-y-auto custom-scrollbar border-t border-neutral-100 pt-3">
              ...
              return (
                <div key={idx} className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-2 font-mono text-[11px]">
                    <span className="text-neutral-500">{toPersianDigits(cat.value)} عدد</span>
                    <span className="text-neutral-400 font-normal">({toPersianDigits(pct)}٪)</span>
                  </div>
                </div>
              );
            })}
        ...
        >
              ...
              <div key={idx} className="space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-1.5 font-mono text-xs">
                    <span className="font-bold text-neutral-900">{toPersianDigits(item.value)} تنوع</span>
                    <span className="text-neutral-400">({toPersianDigits(item.percentage)}٪)</span>
                  </div>
                </div>
                {/* Progress Bar */}
          ...
          <div className="mt-6 p-3 rounded-lg bg-neutral-50 border border-neutral-200/60 text-[11px] text-neutral-600 leading-relaxed">
            <p className="flex items-center gap-1 font-bold text-neutral-800 mb-1">
          ...
          <div className="space-y-4 pt-1">
            {warehouseStockData.length === 0 ? (
              <div className="text-center py-10 text-neutral-400 text-xs">
                <Building2 className="w-8 h-8 mx-auto mb-2 text-neutral-300 stroke-1" />
                <span>هیچ انباری تعریف نشده است.</span>
              </div>
            ) : (
              warehouseStockData.map((wh, idx) => (
                <div key={idx} className="p-3.5 rounded-xl border border-neutral-200/80 bg-neutral-50/50 space-y-2">
                  <div className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center justify-between text-xs pt-1 border-t border-neutral-200/60">
                    <span className="text-neutral-500">موجودی کالا:</span>
                    <span className="font-mono font-bold text-neutral-900 text-[13px]">
                      {toPersianDigits(wh.stockCount)} <span className="text-xs font-normal text-neutral-500">عدد</span>
                    </span>
                  </div>
