 ...
 * - If server responds: Directus server is authoritative. Any module marked revoked/expired
 *   or deleted on server is automatically disabled locally.
 * - If offline: Cryptographically verifies local stored token and permits offline grace operation.
 */
export async function syncOrganizationLicenses(
  ...
  try {
    ...
    // 5. Handle local modules that do NOT exist in Directus (deleted/revoked on server)
    for (const lMod of localModules) {
      if (lMod.slug && !processedSlugs.has(lMod.slug) && lMod.status === 'active') {
        // Module was deleted or not present in Directus organization_modules
        console.warn(`[LicenseSync] Module ${lMod.slug} not found on server for org ${organizationId}. Revoking local access.`);
        revokedCount++;
        if (adapter.saveOrganizationModule) {
          await adapter.saveOrganizationModule({
            ...lMod,
    ...
    if (typeof window !== 'undefined') {
      ...
      // Dispatch global window event so React UI reacts immediately
      window.dispatchEvent(
        new CustomEvent('tankhor_modules_updated', {
          detail: { organizationId, timestamp, syncInfo },
        })
      );
    }
